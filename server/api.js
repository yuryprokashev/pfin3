...
var routes = function routes(wagner) {
    ...
    api.get('/payload/monthData/:targetPeriod', function (req, res) {
        ...
        if (isNoUser === false) {
            manager.manage(req, res).then(manager.returnResult, manager.returnError);
        }
    });
    ...
    var handleNoUser = function handleNoUser(req, res) {
        var reply;
        // console.log(req.user);
    ...
    api.get('/command/:commandType/:dayCode/:payloadType/:commandId', function (req, res) {
        ...
        if (isNoUser === false) {
            manager.manage(req, res).then(manager.returnResultA, manager.returnError);
        }
    });
    ...
    api.get('/command/:commandType/:targetPeriod/:sourcePeriod/:payloadType/:commandId', function (req, res) {
        ...
        if (isNoUser === false) {
            manager.manage(req, res).then(manager.returnCommandResult, manager.returnError);
        }
    });
    ...
    api.post('/bot-' + TG_BOT_TOKEN + '/message/:t', function (req, res) {
        res.json({ success: true, t: req.params.t });
    });
