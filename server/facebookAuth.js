...
function facebookAuth(User, server, Config) {
    ...
    function authenticateWithFB(accessToken, refreshToken, profile, cb) {
        ...
        User.findOneAndUpdate({ 'private.oauth': profile.id }, {
            $set: {
    ...
    server.get(Config.passport.fb.authCallbackPATH, passport.authenticate('facebook', { failureRedirect: Config.passport.fb.failureAuthRedirectPATH }), redirectOnAuthSuccess);
    ...
    server.get(Config.passport.fb.logoutURL, logout);
