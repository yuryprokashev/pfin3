...
MonthSwitch = function MonthSwitch(state) {
    ...
    // return: Bool result
    var isRenewSwitch = function isRenewSwitch() {
        var result = false;
...
MonthSwitch.prototype.moveWindow = function (step) {
    ...
    function createMonth(month) {
        if (!isInCache(month)) {
            var m = new Month(month, _this.state);
    ...
    // MAIN FLOW
    var rng = isMoveToFuture(step) ? [1, step] : [step, -1];
    var target = isMoveToFuture(step) ? lastVisibleMonth() : firstVisibleMonth();
    var createdMonths = this.findNeighbourMonths(target, rng).map(createMonth);
...
//# sourceMappingURL=MonthSwitch.js.map