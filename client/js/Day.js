...
// return: Day object
Day = function Day(dayNum, weekNum, month, state, weekDayNum) {
    var self = this;
    self.state = state;
    ...
    self.initHTML = function () {
        ...
        self.html.sumIf = function (flag, arg, arr) {
            var isFlag = function isFlag(arrItem) {
    ...
    self.update = function () {
        self.setIsInSelectedWeek().setIsSelected().setMaxItems().setIsFuture().setCurrency();
        // .updateItems(state);
    };

    // MAIN LOOP
    self.setUp(dayNum, weekNum, month).initHTML().setCurrency();
};
